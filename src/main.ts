...
export default class EasyTypingPlugin extends Plugin {
	...
	async onload() {
		...
		// Initialize user rules arrays
		this.UserDeleteRules = [];
		this.UserConvertRules = [];

		// Load user rules asynchronously
	...
	formatArticle = async (editor: Editor, view: MarkdownView): Promise<void> => {
		try {
			const { ArticleFormatter } = await import('./modules/article-formatter');
			...
			new Notice('Red Panda Formatter: 載入格式化模組失敗', 3000);
		}
	}
	...
	formatSelectionOrCurLine = async (editor: Editor, view: MarkdownView): Promise<void> => {
		try {
			const { ArticleFormatter } = await import('./modules/article-formatter');
			const formatter = new ArticleFormatter(this.settings);
	...
	deleteBlankLines = async (editor: Editor): Promise<void> => {
		try {
			const { ArticleFormatter } = await import('./modules/article-formatter');
			const formatter = new ArticleFormatter(this.settings);
	...
	private userRulesProcessor: any = null;

	private async loadUserRulesAsync() {
		try {
			const { UserRulesProcessor } = await import('./modules/user-rules');
	...
	async addUserConvertRule(before: string, after: string) {
		this.settings.userConvertRulesStrList.push([before, after]);
		await this.refreshUserConvertRule();
	}

	async deleteUserConvertRule(idx: number) {
	...
	async updateUserConvertRule(idx: number, before: string, after: string) {
		if (idx >= this.settings.userConvertRulesStrList.length || idx < 0) return;
