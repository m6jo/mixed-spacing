      ...
      if (processor.canHandle(context)) {
        ...
        if (result !== context.fullContent) {
          ...
          if (this.debugMode) {
            ...
            console.log(`Original length: ${context.fullContent.length}, New length: ${result.length}`);
          }
...
var DocumentProcessor = class {
  ...
  process(text, context) {
    let result = text;
    console.log(`DocumentProcessor: Processing ${text.length} characters`);
    console.log(`DocumentProcessor: First 100 chars: "${text.substring(0, 100)}"`);
    ;
    ...
    console.log(`DocumentProcessor: Result ${result.length} characters`);
    console.log(`DocumentProcessor: First 100 chars result: "${result.substring(0, 100)}"`);
    return result;
  }
  /**
   * 檢查是否需要為分隔線添加間距
...
// src/processors/document-cleanup-processor.ts
var DocumentCleanupProcessor = class {
  constructor() {
    this.name = "DocumentCleanup";
    this.priority = 15;
    this.type = "document";
  }
  canHandle(context) {
    return /[ \t\u00A0\u200B]+$/m.test(context.fullContent) || /\n{3,}/.test(context.fullContent);
  }
  process(text, context) {
    ...
    const cleanedLines = lines.map((line) => {
      ...
      if (this.isCodeFence(trimmedStart)) {
      ...
      return this.cleanTextLine(line);
    });
    const trimmedEdges = this.trimEmptyEdges(cleanedLines);
    const collapsed = this.collapseBlankRuns(trimmedEdges);
  ...
  cleanTextLine(line) {
    ...
    if (!body)
      return "";
    let trimmedBody = body.replace(/[\u00A0\u200B]+$/g, "").replace(/\s+$/g, "");
    if (!trimmedBody) {
      return leadingWhitespace ? leadingWhitespace : "";
    }
    trimmedBody = this.normalizeListSpacing(trimmedBody);
    trimmedBody = this.compressInlineSpaces(trimmedBody);
    if (this.isMarkdownEmptyList(trimmedBody)) {
      return `${leadingWhitespace}${trimmedBody} `;
    }
    return `${leadingWhitespace}${trimmedBody}`;
  }
  normalizeListSpacing(value) {
    return value.replace(/^([-*+])\s+/, "$1 ").replace(/^(\d+\.)\s+/, "$1 ");
  }
  compressInlineSpaces(value) {
    return value.replace(/ {2,}/g, " ");
  }
  isMarkdownEmptyList(value) {
...
var ParagraphProcessor = class {
  ...
  process(text, context) {
    ...
    if (this.isEmptyListItem(result)) {
      result = result.replace(/\s*$/, " ");
    } else {
      result = result.replace(/\s+$/, "");
    }
  ...
  isEmptyListItem(line) {
    const trimmedStart = line.trimStart();
    return /^[-*+]\s*$/.test(trimmedStart) || /^\d+\.\s*$/.test(trimmedStart);
  }
  /**
...
var CleanupProcessor = class {
  ...
  canHandle(context) {
    return /\s{2,}|\s+$|\s+\u00A0|\u00A0+|\s+\u200B|\u200B+/.test(context.line);
  }
  process(text, context) {
    if (text.includes("1/")) {
      console.log(`[CleanupProcessor] Processing line with "1/": "${text}" (length: ${text.length})`);
      console.log(`[CleanupProcessor] Ends with spaces:`, /\s+$/.test(text));
      console.log(`[CleanupProcessor] Character codes:`, Array.from(text).map((c) => c.charCodeAt(0)));
    }
    if (this.isMarkdownListLine(text)) {
      const trimmedStart = text.trimStart();
      const isEmptyList = /^[-*+]\s*$/.test(trimmedStart) || /^\d+\.\s*$/.test(trimmedStart);
      const result2 = isEmptyList ? text.replace(/\s*$/, " ") : text.replace(/\s+$/g, "");
      if (text.includes("1/")) {
        console.log(`[CleanupProcessor] List line result: "${result2}"`);
      }
      return result2;
    }
    const result = text.replace(/\s{2,}/g, " ").replace(/\s+$/g, "").replace(/\u00A0+/g, "").replace(/\u200B+/g, "");
    if (text.includes("1/")) {
      console.log(`[CleanupProcessor] Final result: "${result}"`);
    }
    return result;
  }
  // 空列表項清理功能已移除（過於激進，影響用戶編輯流程）
  /**
...
var ViewportCleanupProcessor = class {
  ...
  process(text, context) {
    if (this.isMarkdownListLine(text)) {
      const trimmedStart = text.trimStart();
      const isEmptyList = /^[-*+]\s*$/.test(trimmedStart) || /^\d+\.\s*$/.test(trimmedStart);
      return isEmptyList ? text.replace(/\s*$/, " ") : text.replace(/\s+$/g, "");
    }
    return text.replace(/\s{2,}/g, " ").replace(/\s+$/g, "").replace(/\u00A0+/g, "").replace(/\u200B+/g, "");
  }
  // 空列表項清理功能已移除（過於激進，影響用戶編輯流程）
  /**
   * 檢查是否為 Markdown 列表行（需要保護縮排）
...
var RedPandaEditor = class extends import_obsidian.Plugin {
  ...
  registerAllModules() {
    ...
    this.engine.registerProcessor(new DocumentCleanupProcessor());
